  ...
  try {
    ...
    const mediaEntries = await Promise.all(
      assetEntries.map(async (entry) => {
        ...
        const isVideo = /\.(mp4|mov|webm|m4v|ogg|ogv)$/i.test(src);
...
const assetFiles = matches.filter((value) => !value.startsWith('../') && /\.(jpg|jpeg|png|webp|gif|mp4|mov|webm|m4v|ogg|ogv)$/i.test(value));
...
async function loadEntriesFromFolders() {
  const folders = [
    ...
    { key: 'video', orientation: 'landscape' },
  ];
  ...
  for (const folder of folders) {
    ...
    fileNames.forEach((fileName) => {
      const isVideo = /\.(mp4|mov|webm|m4v|ogg|ogv)$/i.test(fileName);
      entries.push({
...
async function loadFolderFileNames(folderPath) {
  ...
  const matches = [...listingHtml.matchAll(/href=["']([^"']+)["']/gi)].map((match) => match[1]);
  return matches.filter((value) => !value.startsWith('../') && /\.(jpg|jpeg|png|webp|gif|mp4|mov|webm|m4v|ogg|ogv)$/i.test(value));
}
